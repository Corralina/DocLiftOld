...
<@dom.dom>
    ...
    <form action="/allResolutionList" method="post" class="shadow-lg mb-5 bg-white rounded p-3 d-flex flex-column">
                ...
                <tr>
                    ...
                    <td><div class="rounded mt-2 shadow-lg mb-2 rounded my_table_row" >
                            <#if resolution?ifExists.status?ifExists.finish??>
                                ${resolution?ifExists.visa?ifExists.data?ifExists}
                            <#else >
                                ${resolution?ifExists.date?ifExists}
                            </#if>
                        </div></td>
                    <td><div class="rounded mt-2 shadow-lg mb-2 rounded my_table_row" >${resolution?ifExists.agrees?ifExists.information?ifExists.individual?ifExists.initials?ifExists}</div></td>
                    <td><div class="rounded mt-2 shadow-lg mb-2 rounded my_table_row" >${resolution?ifExists.filling?ifExists.information?ifExists.individual?ifExists.initials?ifExists}</div></td>
                    ...
                    <td><div class="rounded mt-2 shadow-lg mb-2 rounded my_table_row" ><a class="pathC" href="/resolutionShow/${resolution?ifExists.id?ifExists}">Перегляд</a></div></td>
                </tr>
            </#list>
            </tbody>
        </table>
