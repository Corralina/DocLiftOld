...
<@dom.dom>
    <div class="mt-5 shadow-lg mb-5 bg-white rounded d-flex flex-row" style="min-height: 70vh">
        <div class="d-flex align-items-center justify-content-center" style="width: 60%; position: relative">
            ...
            <form action="/actRegister" method="post" class="px-4 py-3">
                ...
                <div class="form-group mt-4">
                    <label for="exampleDropdownFormEmail1">Коментар до act</label>
                    <input name="coment" value="${act.coment}" type="text" class="form-control" id="exampleDropdownFormEmail1" placeholder="коментар">
                </div>
                ...
                <input type="hidden" class="inpV" value="${act?ifExists.id?ifExists}" name="act">
    ...
    <script>
        ...
        function allPerform(){
            <#list timings?ifExists as timing>
            ...
            z.setAttribute("value", "${timing?ifExists.user?ifExists.username?ifExists}");
            var t = document.createTextNode("${timing?ifExists.user?ifExists.information?ifExists.individual?ifExists.initials?ifExists}");
            z.appendChild(t);
            s.appendChild(z);
            ...
            inp.setAttribute("value","${timing?ifExists.post?ifExists}");
