...
@Controller
@RequestMapping("actPlay")
public class actPlayController {
        ...
        if(act.getAuthor().getUsername().equals(user.getUsername())){
            if (act.getStatus().getDraft()){
                if (!act.getStatus().getFinish()){
                    ...
                    model.addAttribute("error","Данний документ вже підписаний, тому не може бути виконана дана функція");
                }
            }else{
                model.addAttribute("error","Данний документ не поставлений на паузу");
            }
        }else {
